...
/* ---------- 8-pill window:
     Show the anchor (repeat anchor or last completed) AND the appended “next” items.
   ---------------------------------------------------- */
$total = count($items);
$start = 0;
...
// - If we inserted a repeat, the repeat item is at $appendedFrom (same scenario as anchor).
// - Otherwise anchor = last completed scenario in $items (use $lastCompletedIdx).
$anchorIdx = -1;
if ($needRepeat && $appendedFrom < $total) {
  $anchorIdx = $appendedFrom;
} else {
  $anchorIdx = ($lastCompletedIdx >= 0) ? $lastCompletedIdx : -1;
}

// First appended “next” (could be same as anchor if repeat was added)
$firstNextIdx = ($appendedFrom < $total) ? $appendedFrom : -1;

// Aim to center window around the **smaller** of (anchorIdx, firstNextIdx) but
// ensure we include up to firstNextIdx+2 so all 3 “next” pills are visible.
if ($firstNextIdx >= 0) {
  $desiredEnd = min($total-1, $firstNextIdx + 2); // up to the 3rd next item
  // Start far enough back to include 8 pills total, but also ensure the anchor is inside the window.
  $start = max(0, $desiredEnd - 7);

  if ($anchorIdx >= 0 && $anchorIdx < $start) {
    // Pull window back so anchor is included, without exceeding bounds.
    $start = $anchorIdx;
    $end   = min($total-1, $start + 7);
  } else {
    $end = $desiredEnd;
  }
} else {
  // No new nexts appended (rare) → show last 8 items or around anchor if we have one
  if ($anchorIdx >= 0) {
    $start = max(0, $anchorIdx - 4);
    $end   = min($total-1, $start + 7);
  } else {
    $start = max(0, $total - 8);
    $end   = $total - 1;
...
$entry['strip'] = array(
  'latest' => array(
    'index'   => ($lastCompletedIdx>=0 ? $lastCompletedIdx : max(0,$anchorIdx)),
