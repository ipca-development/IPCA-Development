...
return Promise.all([
    ...
    fetch('?api=draft_list').then(function(r){ return r.json(); }) // 👈 NEW
  ])
  .then(function(parts){
    var day = parts[0] || {};
    var rem = parts[1] || {};
    ...
    // --- NEW: load availability before rendering ---
    return applyAvailabilityOverlay(DATA.date || dayStr);
  })
	
	
  .then(function(){
    // now render after availability is loaded
    buildHourHeader();
    renderRows();
  })
  .catch(function(err){
...
function addDbgChip(target,text){
  ...
  s.className='dbgChip'; s.textContent=text; target.appendChild(s);
}
...
/* width helpers – defined once, above renderRows */
function spanWidthMins(mins){
  return (mins / ((H_END - H_START) * 60)) * 100;
}
function minutesBetween(a,b){
  var A=parseServerDateTime(a), B=parseServerDateTime(b);
  return (B - A) / 60000;
}
function widthPct(a,b){return spanWidthMins(minutesBetween(a,b));}
...
function _minsFromStamp(st){
  if(!st) return null;
  var s=String(st).trim(), d=new Date(s.replace(' ','T'));
...
function showPrimaryBarForDevice(d, labelEl){
  var R = null;

  // Prefer a saved reminder
  if (d.reminders && d.reminders.length){
    R = pickPrimaryReminder(d.reminders);
  } else if (d.dev_due_iso) {
    // Fallback to legacy maintenance date from list_day
    R = {
      track_by: 'DATE',
      name: 'Maintenance',
      next_due_date: String(d.dev_due_iso).substr(0,10),
      remaining: (d.dev_days_left!=null ? Number(d.dev_days_left) : null)
    };
  }
  ...
  if (R){
    ...
    // pass current meter if we have it, or null (statusBarFromReminder handles both)
    var current = (d.latest_tacho!=null ? d.latest_tacho :
                  (d.latest_hobbs!=null ? d.latest_hobbs : null));
    holder.innerHTML = statusBarFromReminder(R, current);
    labelEl.appendChild(holder);
  }
}

function showPrimaryBarForStaff(s, labelEl){
  var R = null;

  // Prefer a saved reminder
  if (s.reminders && s.reminders.length){
    R = pickPrimaryReminder(s.reminders);
  } else if (s.active_to) {
    // Fallback to “Active until” from list_day
    R = {
      track_by: 'DATE',
      name: 'Active until',
      next_due_date: String(s.active_to),
      remaining: null
    };
  }
...
function showPrimaryBarForDevice(dev, labelEl){
  ...
  // If the reminder tracks hours, pass the current meter so the progress bar is accurate
  var current = null;
...
function showPrimaryBarForStaff(staff, labelEl){
  ...
  var R = pickPrimaryReminder(staff.reminders);
  if (!R) return;
  var holder = document.createElement('div');
  holder.className = 'expWrap';
  holder.innerHTML = statusBarFromReminder(R, null);
...
for (var di = 0; di < devs.length; di++) {
  (function (d) {
    ...
    addRowCell(function (cell) {
      // click anywhere on the row → start a new reservation on this device
      ...
      for (var i = 0; i < rlist.length; i++) {
        ...
        // paint device "Unavailable" blocks like before
        if (typeof isUnavailable === 'function' && isUnavailable(r)) {
          unav.push({ start: r.start_dt, end: r.end_dt });
          continue;
        }

        // Build DEVICE pill content — FIRST LAST (first student) + "time | mission"
        ...
        var timeTxt = tStart + ' - ' + tEnd;

        var missionTxt = r.mission_code ? r.mission_code
                         : (r.mission_name ? r.mission_name : '');
        var metaTxt = missionTxt ? (timeTxt + ' | ' + missionTxt) : timeTxt;

        // Tooltip lines (kept rich)
var missionFull =
  (r.mission_code && r.mission_name) ? (r.mission_code + ' — ' + r.mission_name) :
...
var tipLines = [
  ...
  'Students: ' + (students.length
    ? students.map(function (s){ return (s.first_name||'') + (s.last_name?(' ' + s.last_name):''); }).join(', ')
    : '—'),
  'Mission: ' + missionFull,
  'Time: ' + timeTxt
];
        ...
        items.push({
          ...
          // custom fields used when rendering the slot
      ...
      // device unavailability background
      if (unav.length) {
	 ...
	 // ---- ALSO add draft items for this device ----
      ...
      for (var di2=0; di2<drafts.length; di2++){
        ...
        // students line (first only, if any)
        var sFirst = (dft.student_ids_csv||'').split(',').filter(Boolean)[0] || '';
        var sName  = (dft.first_student_name || sFirst || '—'); // backend will try to populate first_student_name
        ...
        items.push({
          ...
          tipLines: [
            'DRAFT',
            'Instructor: '+(dft.instructor_name||'—'),
            'Device: ' + (d.dev_name||''),
            'Students: ' + (dft.student_names||'—'),
            'Mission: ' + (missionTxt||'—'),
            'Time: ' + tStart + ' - ' + tEnd
          ],
          _isDraft: true
      ...
      if (items.length) {
        // taller lane height for device pills (44px)
        ...
        for (var pi = 0; pi < packed.items.length; pi++) {
          ...
          // tooltip (same behavior as staff)
...
for(var si=0;si<st.length;si++){
  (function(staff){
    ...
    var label='<div class="l1">'+esc((fname+' '+lname).replace(/\s+/g,' ').trim())+
    ...
    addRowCell(function(cell){
      ...
      cell.addEventListener('click',function(){
        openModalPrefill(null,null,9,0,uid);
      },false);

      /* ---------- Availability overlay (grey blocks) ---------- */
      try{
        var av=(DATA&&DATA.availability)?DATA.availability:null;
        var entry=(av&&av[uid])?av[uid]:null;
        if(entry){
          var mode=entry.mode||'unavailability';
          ...
          for(var r=0;r<rules.length;r++){
            ...
            sM=_clamp(sM,dayStart,dayEnd);
            eM=_clamp(eM,dayStart,dayEnd);
            if(eM>sM)spans.push({s:sM,e:eM});
          }
          spans.sort(function(a,b){return a.s-b.s;});
          var merged=[];
          for(var i=0;i<spans.length;i++){
            if(!merged.length||spans[i].s>merged[merged.length-1].e)
              merged.push({s:spans[i].s,e:spans[i].e});
     ...
     /* ---------- Staff reservations (two-line pill + tooltip, like devices) ---------- */
var rlist=(DATA&&DATA.reservations)?DATA.reservations:[];
var items=[], unavRes=[];
...
for(var x=0;x<rlist.length;x++){
  try{
    ...
    var A=parseServerDateTime(r.start_dt), B=parseServerDateTime(r.end_dt);
    if(isNaN(A)||isNaN(B)){ console.warn('Skipping staff res with bad time', r); continue; }

    // keep “unavailable” backgrounds behavior
    if (typeof isUnavailable==='function' && isUnavailable(r)){
      unavRes.push({ start:r.start_dt, end:r.end_dt });
      continue;
    }
    ...
    for(var s=0;s<students.length;s++){
      fullNames.push((students[s].first_name||'')+(students[s].last_name?(' '+students[s].last_name):''));
    }

    // FIRST LAST (+N) for the first line
    var primaryName='—';
    if (fullNames.length){
      primaryName=fullNames[0];
      if (fullNames.length>1) primaryName+=' +'+(fullNames.length-1);
    }

    // meta line: "time | missionCode"
    var missionShort = r.mission_code || '';
    var metaLine = missionShort ? (labelTime+' | '+missionShort) : labelTime;

    // full mission for tooltip
    ...
    items.push({
      ...
      tipLines: [
        'Instructor: '+((r.instr_first||'')+(r.instr_last?(' '+r.instr_last):'')),
        'Device: '+(r.dev_name||''),
        'Students: '+(fullNames.length?fullNames.join(', '):'—'),
        'Mission: '+missionFull,
        'Time: '+labelTime
      ]
    });
  }catch(e){
    console.error('Staff row render error, skipped record:', e, rlist[x]);
  }
}

// paint “unavailable” backgrounds (unchanged)
if(unavRes.length){
  for(var y=0;y<unavRes.length;y++){
    var seg2=unavRes[y];
    var paint=document.createElement('div');
    paint.style.position='absolute';
    paint.style.left = timeToX(seg2.start)+'%';
    paint.style.width= widthPct(seg2.start, seg2.end)+'%';
    paint.style.top  = '0';
...
// render staff pills using the same geometry/look as device pills
if(items.length){
  var packed=packLanes(items), SLOT_H=44, GAP=6, TOP0=6;   // match device height
  var totalH = TOP0 + packed.lanes*(SLOT_H+GAP) - GAP + TOP0;
  cell.style.minHeight = Math.max(54, totalH)+'px';
  ...
  for(var p=0;p<packed.items.length;p++){
    ...
    var slot=document.createElement('div');
    slot.className = 'slot staff' + (it._isDraft ? ' draft' : '');                    // styled like .slot.dev
    slot.setAttribute('data-force-above','1');
    slot.setAttribute('data-res-id', String(it.res_id));
    slot.style.left = timeToX(it.start)+'%';
    slot.style.width= widthPct(it.start, it.end)+'%';
...
if (typeof attachTooltipBehavior === 'function') attachTooltipBehavior(document);
...
function _stripCodeFromLabel(s){
  // remove leading "1-2-3 — " or "1-2-3 - "
  return String(s||'').replace(/^\s*[0-9]+(?:-[0-9]+)*\s*[—-]\s*/,'').trim();
}
function selectMissionFromReservation(R){
  ...
  if (combined){
    for (var i=0;i<f_mission_sel.options.length;i++){
      ...
      if ((o.value||'').trim() === combined){
        f_mission_sel.value = o.value;
        return;
      }
    }
  }

  // 2) match by code (value or text begins with same code)
  if (wantCode){
    for (var j=0;j<f_mission_sel.options.length;j++){
      ...
      if (_extractCode(ov) === wantCode || _extractCode(ot) === wantCode){
        f_mission_sel.value = f_mission_sel.options[j].value;
        return;
      }
    }
  }

  // 3) match by name (compare text after any leading code)
  if (wantName){
    ...
    for (var k=0;k<f_mission_sel.options.length;k++){
      ...
      if (txtAfter === wantNameNorm){
        f_mission_sel.value = f_mission_sel.options[k].value;
        return;
      }
    }
  }

  // 4) nothing matched → inject current mission so it shows up selected
  if (combined){
    var extra = document.createElement('option');
    extra.value = combined;
    extra.text  = combined;
...
/* ===== DROP-IN: make mission prefill robust (MutationObserver + short guard) ===== */
(function(){
  if (typeof window.selectMissionFromReservation !== 'function') return;

  var _orig = window.selectMissionFromReservation;

  // helpers reuse your existing extract/strip fns
  function _targets(R){
    if (!R) return null;
    var wantCode = (R.mission_code||'').trim();
    var wantName = (R.mission_name||'').trim();
    var combined = (wantCode && wantName) ? (wantCode+' - '+wantName) : (wantCode || wantName || '');
    return { wantCode: wantCode, wantName: wantName, combined: combined };
  }
  function _findMatch(sel, tgt){
    if (!sel || !tgt) return null;
    // 1) exact value
    if (tgt.combined){
      for (var i=0;i<sel.options.length;i++){
        var o=sel.options[i];
        if (!o.disabled && (o.value||'').trim()===tgt.combined) return o;
      }
    }
    // 2) by code
    if (tgt.wantCode){
      for (var j=0;j<sel.options.length;j++){
        var ov=sel.options[j].value||'', ot=sel.options[j].text||'';
        if (_extractCode(ov)===tgt.wantCode || _extractCode(ot)===tgt.wantCode) return sel.options[j];
      }
    }
    // 3) by name (after code)
    if (tgt.wantName){
      var wn=tgt.wantName.toLowerCase();
      for (var k=0;k<sel.options.length;k++){
        var txt=_stripCodeFromLabel(sel.options[k].text||'').toLowerCase();
        if (txt===wn) return sel.options[k];
      }
    }
    return null;
  }

  window.selectMissionFromReservation = function(R){
    // call your original now (works if options are already there)
    _orig(R);

    var sel = document.getElementById('f_mission_sel');
    if (!sel) return;

    var tgt = _targets(R);
    if (!tgt || !tgt.combined) return;

    var userTouched = false;
    var start = Date.now();
    var maxMs = 3500;     // stop after ~3.5s
    var applied = false;

    // stop conditions
    function shouldStop(){
      var modal = document.getElementById('modalWrap');
      var modalHidden = modal && modal.style.display === 'none';
      return userTouched || modalHidden || (Date.now()-start > maxMs);
    }

    // try to apply selection when possible
    function applyIfPossible(){
      if (!sel) return true;
      var m = _findMatch(sel, tgt);
    ...
    if (applyIfPossible()) return;

    // Observe repopulation (innerHTML rebuilds/options changes)
    var obs;
    try{
      obs = new MutationObserver(function(){
        if (applyIfPossible() || shouldStop()){
    ...
    // Also poll briefly in case options are replaced without mutations firing
    var tries = 0;
    var poll = setInterval(function(){
      if (applyIfPossible() || shouldStop() || ++tries>50){
        clearInterval(poll);
        if (obs) try { obs.disconnect(); } catch(e){}
      }
    }, 70);

    // If the user touches the select, stop auto-reapplying
    sel.addEventListener('change', function onUser(){
      userTouched = true;
      clearInterval(poll);
      if (obs) try { obs.disconnect(); } catch(e){}
      sel.removeEventListener('change', onUser);
    }, { once:true });
  };
})();	
  ...
  loadFormOptions(function(){
    fetch('?api=get_reservation&res_id='+encodeURIComponent(resId))
      ...
      .then(function(js){
        ...
        updateMissionField();
		setTimeout(function(){
  if (typeToScenarioKind(f_type.value)) {
    selectMissionFromReservation(R);
  }
}, 0);
      });
...
// ===== Cohort dropdown wiring + render under grid =====
(function(){
  var btn    = document.getElementById('cohortBtn');
  // Guard to prevent the MutationObserver from reacting while we rebuild
  ...
  var allCohorts = [];            // [{id,name,program,...}]
  var fullStudentHTML = null;     // backup of #f_student options before filtering
  var cohortGroupsCache = [];     // [{id,name,users:[{id,first_name,last_name}]}]
  ...
  function loadCohorts(){
    ...
    fetch('?api=cohort_list').then(function(r){return r.json();}).then(function(js){
  ...
  function renderList(){
    ...
    for (var i=0;i<allCohorts.length;i++){
      ...
      row.innerHTML = '<input type="checkbox" id="'+id+'" '+(selected.has(String(c.id))?'checked':'')+'>'
  ...
  document.getElementById('cohortApply').addEventListener('click', function(){
    applyCohorts(); togglePanel(false);
  });
  ...
  /* ===== Modal Student/User filtering (unchanged behavior) ===== */
...
// Build cohort-student reservation pills into a given cell (with tooltip)
function renderReservationsForCohortStudent(cell, userId){
  if (!cell || !DATA || !DATA.reservations) return;

  // collect this student's reservations for the day
  var items = [];
  for (var i=0; i<DATA.reservations.length; i++){
    var r = DATA.reservations[i];
    // check if this reservation includes the student
    var arr = (DATA.res_students && DATA.res_students[r.res_id]) ? DATA.res_students[r.res_id] : [];
    var has = false;
    for (var k=0;k<arr.length;k++){
      if (String(arr[k].userid) === String(userId)) { has = true; break; }
    }
    if (!has) continue;
    ...
    items.push({
      ...
      _metaTxt: timeTxt + (r.mission_code ? (' | ' + r.mission_code) : ''),
      tipLines: tipLines
    });
  }
  ...
  // pack lanes like staff/dev (same geometry)
  var packed = packLanes(items);
  var SLOT_H = 44, GAP = 6, TOP0 = 6;
  var totalH = TOP0 + packed.lanes*(SLOT_H+GAP) - GAP + TOP0;
  cell.style.minHeight = Math.max(54, totalH) + 'px';
  ...
  for (var p=0; p<packed.items.length; p++){
    ...
    var slot = document.createElement('div');
    slot.className = 'slot staff';           // reuse staff styling
    slot.setAttribute('data-force-above','1');
    slot.setAttribute('data-res-id', String(it.res_id));
    slot.style.left  = timeToX(it.start) + '%';
    slot.style.width = widthPct(it.start, it.end) + '%';
    ...
    // 2-line body (student’s name is the row label; pill shows meta)
    var txt = document.createElement('span');
    txt.className = 'slotText';
    txt.innerHTML =
  ...
  // ADD THIS LINE to (re)bind hover tooltips for newly added cohort pills
...
function renderOneCohortStudentRow(cell, uid){
  ...
  cell.addEventListener('click', function(e){
    ...
    openModalPrefill(null, null, hh, mm, null);
    setTimeout(function(){
      var sel = document.getElementById('f_student');
      if (sel){
        for (var i=0;i<sel.options.length;i++){
  ...
  if (items.length){
    ...
    for(var p=0;p<packed.items.length;p++){
      var it=packed.items[p];
		...
		slot.setAttribute('data-force-above','1');   // keep tooltips above & consistent with other rows
		slot.setAttribute('data-res-id', String(it.res_id));
...
function _studentHasReservation(resId, sid){
  var arr = (DATA && DATA.res_students && DATA.res_students[resId]) ? DATA.res_students[resId] : [];
  for (var i=0;i<arr.length;i++){
    if (String(arr[i].userid) === String(sid)) return true; // note: res_students uses "userid"
  }
  return false;
}	
...
function renderCohortStudents(){
  // block observer reactions while we mutate the grid
  ...
  // Each cohort as its own subheader + rows
  for (var i = 0; i < cohortGroupsCache.length; i++) {
    var g = cohortGroupsCache[i];
    addSectionLabel('— ' + g.name); // lightweight subheader
    var users = g.users || [];
    for (var k = 0; k < users.length; k++) {
      var u = users[k];
      var name = (u.first_name || '') + ' ' + (u.last_name || '');
      // Get the cell we will draw into
      var cell = addStudentRow(name);
      // Draw the student’s reservations (with tooltip) into that cell
      renderOneCohortStudentRow(cell, u.id);
    }
  }

  // (Re)bind tooltip behavior for the newly inserted cohort pills
  if (typeof attachTooltipBehavior === 'function') {
    // Bind only within the grid to avoid double-binding the whole document
    attachTooltipBehavior(rowsGrid);
  }
...
/* Re-inject cohort rows when the base grid is rebuilt (e.g., day nav)
   but DO NOT trigger while we ourselves are rendering. */
var mo = new MutationObserver(function(muts){
  if (COHORT_IS_RENDERING) return; // ignore our own DOM writes

  // Only re-render cohorts after the base renderer clears the grid
  // (i.e., when there are no cohort nodes present anymore)
  var hasCohortNodes = !!rowsGrid.querySelector('[data-sec="cohorts"]');
  if (hasCohortNodes) return;

  // slight debounce to let base renderer finish
  if (renderCohortStudents._t) clearTimeout(renderCohortStudents._t);
  renderCohortStudents._t = setTimeout(renderCohortStudents, 0);
});

if (rowsGrid){
  mo.observe(rowsGrid, {childList:true, subtree:false});
}
...
/* =========================================================
   AI Scheduler — Modal open/close (delegated) + basic send
   ========================================================= */
(function(){
  var wrap  = document.getElementById('aiWrap');
  ...
  // Open from ANY button/link with .js-open-ai
  document.addEventListener('click', function(e){
    var trigger = e.target.closest('.js-open-ai');
    if (trigger){
      e.preventDefault();
      openAI();
    }
  ...
  if (wrap){
    wrap.addEventListener('click', function(e){
      if (e.target === wrap) closeAI();
    });
  }

  // ESC key closes it
  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape' && wrap && wrap.classList.contains('show')) closeAI();
  });

  // Ensure aiAppendSystemLine exists (used by your other handlers)
  if (!window.aiAppendSystemLine){
    window.aiAppendSystemLine = function(text){
      if (!chat) return;
      var p = document.createElement('div');
      p.style.cssText = 'margin:8px 0; color:#475569; font-size:13px;';
      p.textContent = text;
  ...
  // Optional: basic send echo (keeps UI responsive without backend)
  if (sendBtn && !sendBtn.__bound){
    sendBtn.__bound = true;
    sendBtn.addEventListener('click', function(){
      var msg = (input.value || '').trim();
      if (!msg) return;
      window.aiAppendSystemLine('You: ' + msg);
      input.value = '';
      // Hook your server call here if desired, then refresh drafts.
      // fetch('?api=ai_schedule', { method:'POST', body: JSON.stringify({ prompt: msg }) })
      //   .then(r=>r.json()).then(()=> { if (window.forceRefresh) forceRefresh(); else if (window.fetchDay) fetchDay(); });
    });
  }
})();	
