    ...
    try{
      ...
      for($i=0;$i<count($rows);$i++){
        ...
        $devRem[$devIndex[$devId]]['reminders'][] = array(
			  ...
			  'due_text'             => $due_text,
			  'remaining'            => $remaining,             // hours or days; may be null
			  'severity'             => $sev,
			  'primary'              => (int)$r['primary_flag']?true:false,
			  // NEW: give the front-end what it needs for % / color
  ...
  if (count($staffIdList)){
    ...
    try{
      $rows = q('SELECT id, target_id, name, track_by,
                  ...
                  ORDER BY primary_flag DESC, updated_at DESC', $staffIdList)->fetchAll();
...
if ($api === 'reminders_save') {
  ...
  try {
    q('INSERT INTO reminders
        (target_type,target_id,name,track_by,
         last_completed_num,last_completed_date,
         interval_value,interval_unit,calendar_month,
         warn_value,warn_unit,
         next_due_num,next_due_date,
  ...
  for(var j=0;j<slots.length;j++){
    (function(slot){
      ...
      slot.addEventListener('click', function(e){
        // Fix #2: if this slot represents a reservation, let the editor open and do nothing here
        if (slot.getAttribute('data-res-id')) return;
        e.stopPropagation();
...
/* ---- Canonicalize device type (single global) ---- */
      ...
      return function(cell){
        ...
        for(i=0;i<rlist.length;i++){
          ...
          if(String(r.device_id)===String(devIdCopy)){
            ...
            // Fix #1: carry res_id in items so we can reference it later
            items.push({
              res_id: r.res_id,
        ...
        if(items.length){
          ...
          for(var pi=0; pi<packed.items.length; pi++){
            ...
            // Fix #1: use it.res_id here
            slot.setAttribute('data-res-id', String(it.res_id));
            slot.addEventListener('click', function(e){
              e.stopPropagation();
              // Use dataset to be safe with closures
      ...
      return function(cell){
        ...
        for(var i=0;i<rlist.length;i++){
          ...
          if(String(r.instructor_user_id)===String(uObj.id)){
            ...
            // Click to edit this reservation
