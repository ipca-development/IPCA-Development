...
if ($api === 'list_day') {
  ...
  /* normalize reminder counters from maint_next (epoch int) */
  $nowTs = time();
  ...
  /* STAFF — roles INSTRUCTOR/ADMIN, active on the selected date (zero date excluded), work_location exactly US */
  $u = $COLMAP_USERS;
  $selU =
    ''.$u['id'].' AS id, '.
    ...
    $u['work_loc'].' AS work_location';

  $staff = array();
  try {
    // active_to must be >= selected date (not just today)
    ...
    $staff = q($userSQL, array($date))->fetchAll();
  ...
  /* Reservations overlapping the selected day */
  $resRows = array();
  $resStudents = array();
  try {
    ...
    if ($resRows && count($resRows)) {
      $ids = array();
      for ($i=0; $i<count($resRows); $i++) { $ids[] = (int)$resRows[$i]['res_id']; }
      ...
      for ($i=0; $i<count($resStudentsRaw); $i++) {
        $r = $resStudentsRaw[$i];
        $rid = (int)$r['res_id'];
        if (!isset($resStudents[$rid])) $resStudents[$rid] = array();
        $resStudents[$rid][] = array(
          'userid'     => $r['userid'],
    ...
    /* =========================== DRAFTS — normalized into reservations =========================== */
    $draftRows = array(); $draftStudents = array(); $draftIds = array();

    try{
      // 1) Load drafts overlapping the day, and JOIN names like real reservations
      //    We synthesize: res_id (negative), status='DRAFT', color_hint='draft' (or any style token), is_draft=1
      $sqlD = 'SELECT 
                  (-100000 - rd.id)            AS res_id,
                  rd.res_type                  AS res_type,
                  rd.start_dt                  AS start_dt,
                  rd.end_dt                    AS end_dt,
                  rd.device_id                 AS device_id,
                  rd.instructor_user_id        AS instructor_user_id,
                  rd.route                     AS route,
                  rd.mission_code              AS mission_code,
                  rd.mission_name              AS mission_name,
                  "DRAFT"                      AS status,
                  "draft"                      AS color_hint,
                  d.'.$d['name'].'             AS dev_name,
                  d.'.$d['type'].'             AS dev_type,
                  d.'.$d['model'].'            AS dev_sort,
                  iu.'.$u['fname'].'           AS instr_first,
                  iu.'.$u['lname'].'           AS instr_last,
                  rd.student_ids               AS _student_ids_csv
               FROM reservation_drafts rd
               LEFT JOIN '.$d['table'].' d  ON d.'.$d['id'].'  = rd.device_id
               LEFT JOIN '.$u['table'].' iu ON iu.'.$u['id'].' = rd.instructor_user_id
               WHERE rd.start_dt <= ? AND rd.end_dt >= ?
               ORDER BY rd.start_dt ASC';

      $draftRows = q($sqlD, array($end,$start))->fetchAll();
      $debug['drafts_sql']   = $sqlD;
      $debug['drafts_count'] = count($draftRows);

      // 2) Build student map for drafts, using the SAME structure as reservations (keyed by res_id)
      //    We’ll gather all student IDs in one pass to avoid N queries where possible.
      $allStu = array(); // flat list of ints
      for($i=0;$i<count($draftRows);$i++){
        $rid = (int)$draftRows[$i]['res_id'];
        $draftIds[] = $rid;

        $csv = isset($draftRows[$i]['_student_ids_csv']) ? trim($draftRows[$i]['_student_ids_csv']) : '';
        if ($csv!==''){
          $parts = preg_split('/\s*,\s*/',$csv);
          for($j=0;$j<count($parts);$j++){
            if($parts[$j]!=='' && ctype_digit($parts[$j])){
              $sid = (int)$parts[$j];
              $allStu[] = $sid;
              // pre-create bucket
              if(!isset($draftStudents[$rid])) $draftStudents[$rid] = array();
              // temporarily stash numeric only; names filled after lookup
              $draftStudents[$rid][] = array('userid'=>$sid,'first_name'=>null,'last_name'=>null);
            }
          }
        }
        // drop helper column so payload stays clean
        unset($draftRows[$i]['_student_ids_csv']);
        // mark as draft for the UI
        $draftRows[$i]['is_draft'] = 1;
      }

      // 3) Name lookup for all unique student IDs found in drafts (if any)
      if(count($allStu)){
        $allStu = array_values(array_unique($allStu));
        $ph = implode(',', array_fill(0, count($allStu), '?'));
        $sqlNames = 'SELECT '.$u['id'].' AS userid, '.$u['fname'].' AS first_name, '.$u['lname'].' AS last_name
                    FROM '.$u['table'].' WHERE '.$u['id'].' IN ('.$ph.')';
        $rowsNames = q($sqlNames, $allStu)->fetchAll();
        // index by userid
        $byId = array();
        for($i=0;$i<count($rowsNames);$i++){
          $byId[(int)$rowsNames[$i]['userid']] = array(
            'first_name'=>$rowsNames[$i]['first_name'],
            'last_name' =>$rowsNames[$i]['last_name']
          );
        }
        // fill names back into $draftStudents
        foreach($draftStudents as $rid=>$arr){
          for($k=0;$k<count($arr);$k++){
            $sid = (int)$arr[$k]['userid'];
            if(isset($byId[$sid])){
              $draftStudents[$rid][$k]['first_name'] = $byId[$sid]['first_name'];
              $draftStudents[$rid][$k]['last_name']  = $byId[$sid]['last_name'];
            }
          }
        }
      }

      // 4) Merge drafts into the main reservation arrays so the UI lanes/packing handle them identically
      if(count($draftRows)){
        // merge rows
        $resRows = array_merge($resRows, $draftRows);

        // merge students map
        foreach($draftStudents as $rid=>$list){
          $resStudents[$rid] = $list; // res_id keys for drafts are negative, so no collision
        }

        // 5) Ensure final sort by start_dt for a coherent lane order
        usort($resRows, function($a,$b){
          $sa = isset($a['start_dt']) ? strtotime($a['start_dt']) : 0;
          $sb = isset($b['start_dt']) ? strtotime($b['start_dt']) : 0;
          if ($sa===$sb) return 0;
          return ($sa<$sb)? -1 : 1;
        });
      }
      ...
      $debug['drafts_error'] = $e->getMessage();
    }	
...
/* ==========================================================
   API: draft_list — return only drafts overlapping the date
   ========================================================== */
if ($api === 'draft_list') {
  header('Content-Type: application/json; charset=utf-8');
  try {
    // 1️⃣ Determine the selected date (default today)
    $date = isset($_GET['date']) && $_GET['date'] ? $_GET['date'] : date('Y-m-d');

    // 2️⃣ Compute day start/end — reuse your helper if present
    ...
    // 3️⃣ Fetch only drafts overlapping that day

	  $rows = q('SELECT d.id,
                  d.res_type,
                  d.device_id,
                  d.instructor_user_id,
                  CONCAT(u.voornaam, " ", u.naam) AS instructor_name,
                  d.student_ids AS student_ids_csv,
                  d.start_dt,
                  d.end_dt,
                  d.mission_code,
                  d.mission_name,
             ...
             LEFT JOIN users u ON u.userid = d.instructor_user_id
            WHERE d.start_dt <= ? AND d.end_dt >= ?
            ORDER BY d.start_dt ASC',
          array($end, $start))->fetchAll();

/* enrich student names + flags (mirrors the logic you had before) */
for ($i = 0; $i < count($rows); $i++) {
  $csv = trim((string)$rows[$i]['student_ids_csv']);
  $rows[$i]['student_names'] = '';
  $rows[$i]['first_student_name'] = null;
  ...
  if ($csv !== '') {
    ...
    if (count($ids)) {
      ...
      if ($names) {
        $rows[$i]['student_names'] = implode(', ', array_map(function($x){ return $x['nm']; }, $names));
        $rows[$i]['first_student_name'] = $names[0]['nm'];
      }
    }
  }
  $rows[$i]['is_draft'] = true;
}
...
if ($api === 'draft_get') {
  ...
  try{
    $r = q('SELECT d.*,
            ...
            WHERE d.id=?
            LIMIT 1', array($id))->fetch();
    if (!$r) jexit(array('ok'=>false,'error'=>'Draft not found'));
    // normalize helpers for UI
    $r['student_ids_csv'] = (string)$r['student_ids'];
    jexit(array('ok'=>true,'draft'=>$r));
  }catch(Exception $e){
    jexit(array('ok'=>false,'error'=>$e->getMessage()));
  }
}	
	
/* ===================== AI: create/update a draft ===================== */
if ($api === 'draft_save') {
  $in = json_decode(file_get_contents('php://input'), true);
  if (!is_array($in)) $in = array();

  $id    = isset($in['id']) ? (int)$in['id'] : 0;
  $type  = isset($in['res_type']) ? trim($in['res_type']) : 'Briefing';
  $start = isset($in['start_dt']) ? trim($in['start_dt']) : null;
  $end   = isset($in['end_dt'])   ? trim($in['end_dt'])   : null;

  if (!$start || !$end) jexit(array('ok'=>false,'error'=>'Missing start/end'));

  $fields = array(
    'res_type'           => $type,
    'start_dt'           => $start,
    'end_dt'             => $end,
    'device_id'          => isset($in['device_id']) ? (int)$in['device_id'] : null,
    'instructor_user_id' => isset($in['instructor_user_id']) ? (int)$in['instructor_user_id'] : null,
    'route'              => isset($in['route']) ? trim($in['route']) : null,
    'mission_code'       => isset($in['mission_code']) ? trim($in['mission_code']) : null,
    'mission_name'       => isset($in['mission_name']) ? trim($in['mission_name']) : null,
    'student_ids'        => isset($in['student_ids']) ? trim($in['student_ids']) : ''
  );

  try{
    if ($id > 0) {
      // UPDATE
      q('UPDATE reservation_drafts
         SET res_type=?, start_dt=?, end_dt=?, device_id=?, instructor_user_id=?,
             route=?, mission_code=?, mission_name=?, student_ids=?
         WHERE id=?',
        array($fields['res_type'],$fields['start_dt'],$fields['end_dt'],$fields['device_id'],
              $fields['instructor_user_id'],$fields['route'],$fields['mission_code'],
              $fields['mission_name'],$fields['student_ids'],$id));
    } else {
      // INSERT
      q('INSERT INTO reservation_drafts
           (created_by, created_at, res_type, start_dt, end_dt, device_id, instructor_user_id,
            route, mission_code, mission_name, student_ids)
         VALUES (NULL, NOW(), ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        array($fields['res_type'],$fields['start_dt'],$fields['end_dt'],$fields['device_id'],
              $fields['instructor_user_id'],$fields['route'],$fields['mission_code'],
              $fields['mission_name'],$fields['student_ids']));
...
if ($api === 'draft_delete') {
  ...
  if (!$id) jexit(array('ok'=>false,'error'=>'Missing id'));
  try{
    q('DELETE FROM reservation_drafts WHERE id=?', array($id));
    jexit(array('ok'=>true));
  }catch(Exception $e){
    jexit(array('ok'=>false,'error'=>$e->getMessage()));
  }
...
/* ===================== AI: convert one draft to real reservation ===================== */
if ($api === 'draft_convert') {
  $in = json_decode(file_get_contents('php://input'), true);
  if (!is_array($in)) $in = array();
  $id = isset($in['id']) ? (int)$in['id'] : 0;
  if (!$id) jexit(array('ok'=>false,'error'=>'Missing id'));
  ...
  try{
    $d = q('SELECT * FROM reservation_drafts WHERE id=? LIMIT 1', array($id))->fetch();
    if (!$d) jexit(array('ok'=>false,'error'=>'Draft not found'));

    // Build INSERT (same logic you use in draft_finalize)
    $cols = array('res_type','start_dt','end_dt','device_id','instructor_user_id','route','mission_code','mission_name','created_at');
    $vals = array($d['res_type'],$d['start_dt'],$d['end_dt'],$d['device_id'],$d['instructor_user_id'],$d['route'],$d['mission_code'],$d['mission_name']);
    $ph   = array('?,?,?,?,?,?,?,?','NOW()');

    $sql = 'INSERT INTO '.$GLOBALS['TBL_RES'].' ('.implode(',',$cols).') VALUES ('.implode(',',$ph).')';
    q($sql,$vals);
    ...
    if ($csv!==''){
      foreach (explode(',',$csv) as $u){
        $uid = (int)trim($u);
        if ($uid>0) q('INSERT INTO '.$GLOBALS['TBL_RS'].' (res_id, student_user_id) VALUES (?,?)', array($newId,$uid));
      }
    }
    ...
    jexit(array('ok'=>true,'res_id'=>$newId));
  }catch(Exception $e){
    jexit(array('ok'=>false,'error'=>$e->getMessage()));
  }
}	
...
if ($api === 'draft_finalize') {
  try {
    // Pull drafts in chronological order
    ...
    // Helper: does an array key exist and hold a non-empty, non-null value?
    $hasval = function($arr, $key) {
      return (is_array($arr) && array_key_exists($key, $arr) && $arr[$key] !== '' && $arr[$key] !== null);
    };

    // Try to detect if the reservations_students table uses "student_user_id" (as used elsewhere)
    // We assume your schema as per S-04 list_day: rs.student_user_id
    $rsStudentCol = 'student_user_id';

    // Optional: quick prepared statements (older PHP 5.3 style retained via q())
    foreach ($drafts as $d) {
      try {
        // Extract/normalize
        $res_type   = isset($d['res_type'])            ? $d['res_type']            : null;
        $start_dt   = isset($d['start_dt'])            ? $d['start_dt']            : null;
        $end_dt     = isset($d['end_dt'])              ? $d['end_dt']              : null;
        $device_id  = isset($d['device_id'])           ? (int)$d['device_id']      : null;
        $instr_id   = isset($d['instructor_user_id'])  ? (int)$d['instructor_user_id'] : null;
        $route      = isset($d['route'])               ? $d['route']               : null;
        $mission_code = isset($d['mission_code'])      ? $d['mission_code']        : null;
        $mission_name = isset($d['mission_name'])      ? $d['mission_name']        : null;

        // Resolve scenario_id, if possible:
        // 1) If reservation_drafts.scenario_id exists and is > 0, use it
        // 2) Else, try unique lookup by mission_code in scenarios (optional)
        $scenario_id = null;
        if ($hasval($d, 'scenario_id')) {
          $scenario_id = (int)$d['scenario_id'] > 0 ? (int)$d['scenario_id'] : null;
        } elseif ($mission_code) {
          // Attempt a unique lookup by mission_code (only if it yields exactly 1 row)
          try {
            $row = q('SELECT sc_id FROM scenarios WHERE mission_code = ? LIMIT 2', array($mission_code))->fetchAll();
            if (is_array($row) && count($row) === 1 && isset($row[0]['sc_id'])) {
              $scenario_id = (int)$row[0]['sc_id'];
            }
          } catch (Exception $e) {
            // Ignore lookup errors; scenario_id remains null
          }
        }

        // Prevent obvious duplicates (same device & timeslot)
        $dupe = q('SELECT res_id FROM '.$GLOBALS['TBL_RES'].' WHERE device_id = ? AND start_dt = ? AND end_dt = ? LIMIT 1',
                  array($device_id, $start_dt, $end_dt))->fetch();
        if ($dupe && isset($dupe['res_id'])) {
          // Consider duplicate a failure to keep behavior explicit
          $fail++;
          $errs[] = 'Duplicate timeslot for device '.$device_id.' at '.$start_dt.'–'.$end_dt.' (draft id '.(isset($d['id'])?$d['id']:'?').')';
          continue;
        }

        // Build INSERT dynamically to include scenario_id only when we have it
        $cols = array('res_type','start_dt','end_dt','device_id','instructor_user_id','route','mission_code','mission_name','created_at');
        $vals = array($res_type, $start_dt, $end_dt, $device_id, $instr_id, $route, $mission_code, $mission_name);
        $ph   = array('?,?,?,?,?,?,?,?', 'NOW()'); // NOW() inline

        if (!is_null($scenario_id)) {
          // Insert scenario_id just before created_at for neatness
          array_splice($cols, count($cols)-1, 0, 'scenario_id');
          array_splice($vals, count($vals),    0, $scenario_id);
          array_splice($ph,   count($ph)-1,    0, '?');
        }
        ...
        // Attach students from CSV (reservation_drafts.student_ids)
        $csv = isset($d['student_ids']) ? trim((string)$d['student_ids']) : '';
        if ($csv !== '') {
          $ids = explode(',', $csv);
          for ($i=0; $i<count($ids); $i++) {
            $uid = (int)trim($ids[$i]);
            if ($uid > 0) {
              q('INSERT INTO '.$GLOBALS['TBL_RS'].' (res_id, '.$rsStudentCol.') VALUES (?,?)', array($newId, $uid));
            }
          }
        }

        $ok++;
        $ok_ids[] = isset($d['id']) ? (int)$d['id'] : null;
      } catch (Exception $e) {
        $fail++;
        $errs[] = $e->getMessage();
      }
    ...
    // Truncate drafts ONLY if all succeeded; otherwise leave as-is for review
    if ($fail === 0) {
      q('TRUNCATE TABLE reservation_drafts');
    }
...
<button id="gridToggle" class="btn" data-on="1">Grid: On</button> 	  
  ...
  <button class="btn ghost" id="btnFinalizeAll">Finalize All Drafts</button>	  
...
function fetchDay(){
  ...
  return Promise.all([
    ...
    fetch('?api=draft_list&date=' + encodeURIComponent(dayStr)).then(function(r){ return r.json(); })
  ...
  .then(function(parts){
    ...
    // 1) Merge reminders into base day payload
    DATA = mergeRemindersIntoDay(day, rem);

    
	// --- 2) Pick drafts strictly overlapping selected day (local window)
var listDraftsRaw = (function(){
  // Accept a few common shapes
  if (drafts && drafts.ok && Array.isArray(drafts.drafts)) return drafts.drafts;
  if (Array.isArray(drafts)) return drafts;                 // endpoint returns array directly
  if (drafts && Array.isArray(drafts.items)) return drafts.items;
  if (drafts && Array.isArray(drafts.data))  return drafts.data;
  return [];
})();

// TEMP: log what we actually got from the endpoint
console.log('[sched] raw drafts payload:', drafts);

// Normalize fields
var listDrafts = listDraftsRaw.map(function(d){
  var out = Object.assign({}, d);

  // Normalize datetime fields to start_dt / end_dt
  if (!out.start_dt) out.start_dt = d.start_dt || d.start || d.start_time || d.begin || '';
  if (!out.end_dt)   out.end_dt   = d.end_dt   || d.end   || d.end_time   || d.finish || '';

  // Normalize staff/device keys
  if (out.instructor_id && !out.instructor_user_id) out.instructor_user_id = out.instructor_id;
  if (out.staff_id && !out.instructor_user_id) out.instructor_user_id = out.staff_id;
  if (out.dev_id && !out.device_id) out.device_id = out.dev_id;
  if (out.device && !out.device_id) out.device_id = out.device.id || out.device;

  // Normalize mission fields
  if (!out.mission_code) out.mission_code = d.mission_code || d.code || d.mission || '';
  if (!out.mission_name) out.mission_name = d.mission_name || d.name || d.title || '';

  // Normalize students
  if (!out.student_ids_csv) {
    if (Array.isArray(d.student_ids)) out.student_ids_csv = d.student_ids.join(',');
    else if (Array.isArray(d.students)) out.student_ids_csv = d.students.map(function(s){return s.id||s.userid||s.user_id||'';}).filter(Boolean).join(',');
    else out.student_ids_csv = String(d.student_ids_csv||'');
  }
  if (!out.student_names && Array.isArray(d.students)) {
    out.student_names = d.students.map(function(s){
      var fn = s.first_name || s.first || s.givenName || '';
      var ln = s.last_name  || s.last  || s.familyName || '';
      return (fn + ' ' + ln).trim();
    }).filter(Boolean).join(', ');
  }
...
var dayISO = (day && day.date) ? day.date : (DATA && DATA.date) ? DATA.date : dayStr;

// Date-only overlap (robust): keep if the draft's date range touches dayISO by date
function dateOnly(s){ return String(s||'').slice(0,10); }
function overlapsDateOnly(s, e, day){
  var sd = dateOnly(s), ed = dateOnly(e);
  if (!sd && !ed) return false;                   // nothing to compare
  if (!sd) sd = ed; if (!ed) ed = sd;             // single date provided
  // Handle reversed or equal
  if (ed < sd) { var t = sd; sd = ed; ed = t; }
  // overlap if: sd <= day <= ed
  return (sd <= day) && (day <= ed);
}

var beforeFilterCount = listDrafts.length;
listDrafts = listDrafts.filter(function(d){ return overlapsDateOnly(d.start_dt, d.end_dt, dayISO); });

// If we filtered everything out, fall back to showing all drafts for visibility (temporary safety net)
if (!listDrafts.length && beforeFilterCount > 0) {
  console.warn('[sched] All drafts filtered out by date. Falling back to show all for debug.');
  listDrafts = listDraftsRaw.map(function(x){ return Object.assign({}, x); }); // unfiltered
}

console.log('[sched] drafts normalized:', listDrafts);  
	  
	  
	  
	  // ADD THIS NORMALIZER
listDrafts = listDrafts.map(function(d){
  var out = Object.assign({}, d);

  // Normalize datetime fields to start_dt / end_dt
  if (!out.start_dt) out.start_dt = d.start_dt || d.start || d.start_time || '';
  if (!out.end_dt)   out.end_dt   = d.end_dt   || d.end   || d.end_time   || '';
  ...
  // Normalize mission fields
  if (!out.mission_code) out.mission_code = d.mission_code || d.code || '';
  if (!out.mission_name) out.mission_name = d.mission_name || d.name || '';

  // Normalize students
  if (!out.student_ids_csv && Array.isArray(d.student_ids)) out.student_ids_csv = d.student_ids.join(',');
  if (!out.student_names && Array.isArray(d.student_names_arr)) out.student_names = d.student_names_arr.join(', ');
 ...
 // Compare only the YYYY-MM-DD part to avoid TZ shifts
listDrafts = listDrafts.filter(function(d){
  var sd = String(d.start_dt || '').slice(0, 10); // 'YYYY-MM-DD'
  var ed = String(d.end_dt   || '').slice(0, 10);
  return (sd === dayISO || ed === dayISO);
});

    // 3) Fallback drafts embedded in list_day (rare)
    var dayDrafts = [];
    try{
      var R = Array.isArray(day.reservations) ? day.reservations : [];
      dayDrafts = R.filter(function(r){
        var isDraft = (r && ((r.is_draft === 1) || ((r.status||'').toUpperCase() === 'DRAFT')));
        return isDraft && _overlapsDay(r.start_dt, r.end_dt);
      }).map(function(r){
        return {
          id:           r.res_id || ('vdraft_'+Math.random().toString(36).slice(2)),
          device_id:    r.device_id || null,
          instructor_user_id: r.instructor_user_id || null,
          instructor_name: ((r.instr_first||'') + (r.instr_last?(' '+r.instr_last):'')).trim() || '',
          student_ids_csv: (function(){
            var arr = (DATA && DATA.res_students && DATA.res_students[r.res_id]) ? DATA.res_students[r.res_id] : [];
            var ids = arr.map(function(s){ return (s.userid!=null ? String(s.userid) : ''); }).filter(Boolean);
            return ids.join(',');
          })(),
          student_names: (function(){
            var arr = (DATA && DATA.res_students && DATA.res_students[r.res_id]) ? DATA.res_students[r.res_id] : [];
            var names = arr.map(function(s){ return ((s.first_name||'') + (s.last_name?(' '+s.last_name):'')).trim(); }).filter(Boolean);
            return names.join(', ');
          })(),
          start_dt:     r.start_dt,
          end_dt:       r.end_dt,
          ...
          res_type:     r.res_type || 'Briefing'
        };
      });
    }catch(_){ dayDrafts = []; }

    // 4) Use API drafts if present; else fallback
    DATA.drafts = listDrafts.length ? listDrafts : dayDrafts;

    // 4.5) Mirror drafts into reservations (negative ids), so all rows see them
    (function mirrorDraftsIntoReservations(){
      if (!DATA) return;
      var drafts = Array.isArray(DATA.drafts) ? DATA.drafts : [];
      if (!drafts.length) return;

      var devById = {};
      (DATA.devices || []).forEach(function(d){ devById[String(d.dev_id)] = d; });

      var vRes = [];
      var vStu = DATA.res_students || {};
      var rid = -100000;

      drafts.forEach(function(d){
        var dev = devById[String(d.device_id)] || null;

        var iFirst = '', iLast = '';
        if (d.instructor_first && d.instructor_last){
          iFirst = d.instructor_first; iLast = d.instructor_last;
        } else if (d.instructor_name){
          var parts = String(d.instructor_name).trim().split(/\s+/);
          iFirst = parts[0] || ''; iLast = parts.slice(1).join(' ');
        }

        var thisId = rid--;
        vRes.push({
          res_id: thisId,
          res_type: d.res_type || 'Briefing',
          start_dt: d.start_dt,
          end_dt:   d.end_dt,
          device_id: d.device_id || null,
          instructor_user_id: d.instructor_user_id || null,
          mission_code: d.mission_code || '',
          mission_name: d.mission_name || '',
          route: d.route || null,
          status: 'DRAFT',
          color_hint: 'draft',
          dev_name: dev ? (dev.dev_name || '') : '',
          dev_type: dev ? (dev.dev_type || '') : '',
          dev_sort: dev ? (dev.dev_model || '') : '',
          instr_first: iFirst,
          instr_last:  iLast,
          is_draft: 1
        });

        var ids = (String(d.student_ids_csv||'').split(',')
                  .map(function(s){ return parseInt(s,10); })
                  .filter(function(n){ return !isNaN(n); })) || [];

        var names = String(d.student_names||'').split(',')
                     .map(function(s){ return s.trim(); })
                     .filter(Boolean);

        vStu[thisId] = ids.map(function(uid, idx){
          var nm = names[idx] || '';
          var first = nm.split(' ')[0] || '';
          var last  = nm.split(' ').slice(1).join(' ') || '';
          return { userid: uid, first_name: first, last_name: last };
        });
      });

      DATA.reservations = (DATA.reservations || []).concat(vRes);
      DATA.res_students  = vStu;
    })();

    console.log('[sched] date=', (DATA.date || dayStr),
                'reservations=', (Array.isArray(DATA.reservations)?DATA.reservations.length:0),
                'drafts(used)=', (DATA.drafts?DATA.drafts.length:0));

    // 5) Update date header + picker
    ...
    // 6) Load availability before rendering (non-blocking)
  ...
  (function(d){
    ...
    addRowCell(function(cell){
      ...
      for(var i=0;i<rlist.length;i++){
        ...
        // Background unavailability rows
        if(isUnavailable(r)){unav.push({start:r.start_dt,end:r.end_dt});continue;}

        // Normalize draft flag
        var st=(r.status||'').toString().toUpperCase();
        var isDraft = (r.is_draft===1) || (st==='DRAFT');
        ...
        if(students.length){
          var s0=students[0];
          primaryName=((s0.first_name||'')+(s0.last_name?(' '+s0.last_name):'')).trim()||'—';
        }
        ...
        var tipLines=[
          ...
          'Students: '+(students.length?students.map(function(s){return(s.first_name||'')+(s.last_name?(' '+s.last_name):'');}).join(', '):'—'),
          'Mission: '+missionFull,
          'Time: '+tStart+' - '+tEnd
        ];
        if (isDraft) tipLines.unshift('DRAFT');
      ...
      // Paint unavailability background
      ...
      if(items.length){
        ...
        for(var pi=0;pi<packed.items.length;pi++){
          ...
          slot.addEventListener('click',function(e){
            ...
            if(!id)return;
            // disable editor for drafts
            if(id.indexOf('-')===0){ return; } // negative ids (drafts) — ignore
  ...
  (function (staff) {
    ...
    addRowCell(function (cell) {
      ...
      try {
        ...
        var entry = _getAvailEntry(uid);
        if (entry) paintAvailabilityIntoCell(cell, entry, H_START, H_END);
      } catch (err) { console.warn('Availability overlay error', err); }

      /* ---------- Staff reservations (incl. drafts) ---------- */
      var rlist = (DATA && DATA.reservations) ? DATA.reservations : [];
      var items = [];
      ...
      for (var x = 0; x < rlist.length; x++) {
        ...
        var stU = (r.status||'').toString().toUpperCase();
        var isDraft = (r.is_draft===1) || (stU === 'DRAFT');
      ...
      if (items.length) {
        ...
        for (var p = 0; p < packed.items.length; p++) {
          ...
          var slot = document.createElement('div');
          slot.className = 'slot staff' + (it._isDraft ? ' draft' : '');
          slot.setAttribute('data-force-above', '1');
          slot.setAttribute('data-res-id', String(it.res_id));
          slot.style.left = timeToX(it.start) + '%';
          slot.style.width = widthPct(it.start, it.end) + '%';
          ...
          slot.addEventListener('click', function (e) {
            ...
            var id = this.getAttribute('data-res-id');
            if (!id) return;
            if (id.indexOf('-')===0) return; // draft → no editor
            openReservationEditor(parseInt(id, 10));
          }, false);
...
// ============================================================
// AI DRAFT INTEGRATION — Edit / Save / Convert / Finalize
// ============================================================
...
// 🔹 When creating draft pills, each should have data-draft-id and click handler
function onDraftClick(ev){
  ev.stopPropagation();
  const el = ev.currentTarget;
  const id = el?.dataset?.draftId ? parseInt(el.dataset.draftId,10) : 0;
  if(!id) return;
  fetch('schedule.php?api=draft_get&id='+id)
    .then(r=>r.json())
    .then(js=>{
      if(!js.ok) return toast(js.error||'Failed to load draft');
      openReservationModalDraft(js.draft);
    })
    .catch(e=>toast('Error: '+e));
}

// 🔹 Function to open modal in DRAFT mode
function openReservationModalDraft(d){
  // Reuse existing modal skeleton if you have one
  const modal=document.querySelector('.modal');
  const title=modal.querySelector('.modalHd h3');
  title.textContent='Edit Draft #' + d.id;

  // fill fields (adapt field IDs if different)
  setVal('#f_type', d.res_type||'Briefing');
  setVal('#f_start_date', d.start_dt.split(' ')[0]);
  setVal('#f_start_time', d.start_dt.split(' ')[1].slice(0,5));
  setVal('#f_end_date', d.end_dt.split(' ')[0]);
  setVal('#f_end_time', d.end_dt.split(' ')[1].slice(0,5));
  setVal('#f_device', d.device_id||'');
  setVal('#f_instructor', d.instructor_user_id||'');
  setVal('#f_route', d.route||'');
  setVal('#f_mission_code', d.mission_code||'');
  setVal('#f_mission_name', d.mission_name||'');
  setVal('#f_students_csv', d.student_ids||'');

  // Rebuild footer buttons
  const ft=modal.querySelector('.modalFt');
  ft.innerHTML='';
  ft.appendChild(makeBtn('Cancel','btnSec',closeModal));
  ft.appendChild(makeBtn('Save Draft','btnSec',()=>saveDraft(d.id)));
  ft.appendChild(makeBtn('Convert to Reservation','btnPri',()=>convertDraft(d.id)));
  ft.appendChild(makeBtn('Delete Draft','btnSec',()=>deleteDraft(d.id)));
...
function getVal(sel){const e=document.querySelector(sel); return e?e.value.trim():'';}

// 🔹 Save draft
function saveDraft(id){
  const p={
    id:id,
    res_type:getVal('#f_type'),
    start_dt:getVal('#f_start_date')+' '+getVal('#f_start_time')+':00',
    end_dt:getVal('#f_end_date')+' '+getVal('#f_end_time')+':00',
    device_id:getVal('#f_device'),
    instructor_user_id:getVal('#f_instructor'),
    route:getVal('#f_route'),
    mission_code:getVal('#f_mission_code'),
    mission_name:getVal('#f_mission_name'),
    student_ids:getVal('#f_students_csv')
  };
  fetch('schedule.php?api=draft_save',{
    method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify(p)
  }).then(r=>r.json()).then(js=>{
    if(!js.ok) return toast(js.error||'Save failed');
    toast('Draft saved');
    closeModal();
    refreshDraftsForSelectedDay();
  });
}

// 🔹 Convert single draft
function convertDraft(id){
  if(!confirm('Convert this draft into a real reservation?')) return;
  fetch('schedule.php?api=draft_convert',{
    method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({id:id})
  }).then(r=>r.json()).then(js=>{
    if(!js.ok) return toast(js.error||'Convert failed');
    toast('Draft converted');
    closeModal();
    Promise.all([refreshReservationsForSelectedDay(),refreshDraftsForSelectedDay()]);
  });
}
...
function deleteDraft(id){
  ...
  fetch('schedule.php?api=draft_delete',{
    method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({id:id})
  }).then(r=>r.json()).then(js=>{
    if(!js.ok) return toast(js.error||'Delete failed');
    toast('Draft deleted');
    closeModal();
    refreshDraftsForSelectedDay();
  });
...
// 🔹 Close modal helper (reuse your existing version if present)
function closeModal(){
  const m=document.querySelector('.modal');
  if(m) m.style.display='none';
}

// 🔹 Finalize all drafts at once
document.querySelector('#btnFinalizeAll')?.addEventListener('click',()=>{
  if(!confirm('Convert ALL current drafts into real reservations?')) return;
  fetch('schedule.php?api=draft_finalize')
    .then(r=>r.json())
    .then(js=>{
      if(!js.ok) return toast(js.error||'Finalize failed');
      toast(js.message||'All drafts finalized');
      Promise.all([refreshReservationsForSelectedDay(),refreshDraftsForSelectedDay()]);
    });
});

// 🔹 Refresh helpers
async function refreshDraftsForSelectedDay(){
  const url='schedule.php?api=draft_list&date='+encodeURIComponent(CUR_DATE||new Date().toISOString().slice(0,10));
  const r=await fetch(url); const js=await r.json();
  if(js.ok){ DATA.drafts=js.drafts||[]; redraw(); }
}
async function refreshReservationsForSelectedDay(){
  const url='schedule.php?api=list_day&date='+encodeURIComponent(CUR_DATE||new Date().toISOString().slice(0,10));
  const r=await fetch(url); const js=await r.json();
  if(js.ok){ DATA.reservations=js.reservations||[]; redraw(); }
}	
	
// === Draft click/edit/finalize ===
function toast(msg){ /* ...as I sent... */ }
function onDraftClick(ev){ /* ... */ }
function openReservationModalDraft(d){ /* ... */ }
function makeBtn(label,cls,fn){ /* ... */ }
