  ...
  (function(){
    ...
    if (btnSave){
      btnSave.addEventListener('click', function(){
        ...
        // Prefer computed “Next Due” if present, otherwise the raw “Due @ …”
        ...
        var payload = {
          ...
          target_type: fTargetType.value,                 // "DEVICE" | "STAFF"
          target_id:   parseInt(fTargetId.value,10),

          name:        fName.value.trim(),
          short_label: (fShortLabel.value || '').trim().substring(0,4),

          track_by:    fTrackBy.value,                    // "HOURS_TACHO" | "HOURS_HOBBS" | "DATE"

          // Raw “due” inputs (kept for server logic / validation)
          due_num:     fnum(fDueNum.value),
          due_date:    (fDueDate.value || null),

          // Last completed — only one is relevant depending on track_by
          ...
          interval_unit:  document.getElementById('remIntervalUnit').value,      // "HOURS" | "DAYS" | "MONTHS"
          ...
          warn_unit:  document.getElementById('remWarnUnit').value,               // "HOURS_TACHO" | "HOURS_HOBBS" | "DAYS" | "MONTHS"

          // Computed “next due” (server may recompute; this helps UX)
        ...
        .then(function(js){
          if (js && js.ok){
            ...
            if (typeof window.location !== 'undefined'){
              // Re-use the loader from R-08
              if (typeof window.DEVICES !== 'undefined' || typeof window.STAFF !== 'undefined'){
                // Trigger the load() function by reloading the page section
                // simplest: reload the whole page to keep state consistent
