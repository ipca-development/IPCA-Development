...
<head>
  ...
  <style>
    ...
    *{ box-sizing:border-box }
    ...
    .btnSec{ background:#f3f6fb; border:1px solid #cfd5e3; color:#1a1f36; padding:8px 12px; border-radius:8px; cursor:pointer; text-decoration:none; display:inline-block; }
...
<body>
  ...
  <div class="wrap"><!-- open main wrapper -->
    ...
    <div class="card" style="margin-bottom:16px;">
      <div class="headerBar" style="padding:10px 12px;">
        ...
        <a href="fuel_station.php" class="btnSec" id="manageFuelBtn">Manage Fuel Station</a>
      </div>
      <div class="pad" style="padding-top:10px;">
        <div id="fuelBar"></div>
        <div id="fuelMeta" class="muted" style="margin-top:6px;"></div>
      </div>
    </div>
  ...
  (function(){
    ...
    // default capacity in gallons (adjust to your tank size)
    var FUEL_CAPACITY = 500.0;
    ...
    // Fuel render — same visual language as device bars
function renderFuel(f){
  if (!fuelMeta) return;

  // No fuel data
  if (!f || !f.has) {
    fuelMeta.innerHTML = '<span class="muted">No fuel reading available.</span>';
    return;
  }

  // Gallons available (like old version)
  var g;
  if (typeof f.gallons === 'number') {
    g = f.gallons;
  } else {
    g = parseFloat(f.gallons || 'NaN');
  }
  var gLabel = isNaN(g) ? '—' : g.toFixed(1);

  // Optional capacity + min level if your API sends them
  var cap = (typeof f.capacity === 'number') ? f.capacity : null;
  var min = (typeof f.min_level === 'number') ? f.min_level : null;

  var pct = 0;
  if (!isNaN(g) && cap && cap > 0) {
    pct = Math.max(0, Math.min(100, (g / cap) * 100));
  }

  // Color logic similar to fuel_station.php
  ...
  var at  = f.updated_at ? new Date(String(f.updated_at).replace(' ','T')).toLocaleString() : '';
  var when = at ? (' on ' + esc(at)) : '';

  // First line: bar, same width as reminder status bars
  // Second line: clear “123.4 gallons by X on Y” text
  fuelMeta.innerHTML =
    '<div style="display:flex; align-items:center; gap:10px; margin-bottom:4px;">' +
      '<div style="flex:0 0 40%; max-width:340px; min-width:200px;' +
                  ' height:10px; background:#e6e9ef; border-radius:999px; overflow:hidden;">' +
        '<div style="width:' + pct + '%; height:100%; background:' + color + '; border-radius:999px;"></div>' +
      '</div>' +
    '</div>' +
    '<div class="muted" style="font-size:12px;">' +
      '<strong>' + esc(gLabel) + ' gallons</strong>' + by + when +
    '</div>';
}
    function load(){
      fetch('schedule.php?api=reminders_list')
        ...
        .catch(function(){
          ...
          if(fuelBar)  fuelBar.innerHTML='<span class="muted">Failed to load fuel status.</span>';
          if(fuelMeta) fuelMeta.innerHTML='';
        });
    }
    load();
  })();
  ...
  (function(){
    ...
    var fShortLabel = document.getElementById('remShortLabel');
    ...
    if(btnOpen){
      btnOpen.addEventListener('click', function(){ openCreate(); }, false);
    }
    ...
    // Load device / staff options for dropdowns
    function loadOptions(cb){
      fetch('schedule.php?api=reminders_form_options')
        ...
        .catch(function(){
          alert('Could not load devices / staff for reminders (reminders_form_options).');
        });
    }
    ...
    function openCreate(){
      ...
      resetForm();
      show();                 // show immediately (fixes "button does nothing" feeling)
      loadOptions(function(){ // async populate selects
        deriveCurrent();
      });
    }
    ...
    window.openEditReminder = function(kind, id){
      ...
      loadOptions(function(js){
        ...
        fetch('schedule.php?api=reminders_get&id='+encodeURIComponent(id))
          ...
          .catch(function(){
            alert('Could not load reminder details (reminders_get).');
          });
      });
    };
    ...
    if (btnSave){
      btnSave.addEventListener('click', function(){
        ...
        var nextNum  = fnum(document.getElementById('remNextNum').value);
        if (nextNum === null) nextNum = fnum(fDueNum.value);

        var payload = {
          id: (CURRENT_REM_ID!=null ? CURRENT_REM_ID : null),

          target_type: fTargetType.value,
          target_id:   parseInt(fTargetId.value,10),

          name:        fName.value.trim(),
          short_label: (fShortLabel.value || '').trim().substring(0,4),
          ...
          next_due_num:  (fTrackBy.value==='DATE') ? null : (nextNum !== null ? nextNum : null),
        ...
        fetch('schedule.php?api=reminders_save', {
          method:'POST',
        ...
        .then(function(js){
          if (js && js.ok){
            ...
            alert('Save failed'+(js&&js.error?(': '+js.error):'.'));
          }
        })
        .catch(function(){
          alert('Save failed (reminders_save).');
        });
      }, false);
    }
  })();
